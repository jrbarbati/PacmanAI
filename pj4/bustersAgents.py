...
class BustersAgent:
    ...
    def __init__(self, index = 0, inference = "ExactInference",\
                 ghostAgents = None, observeEnable = True,\
...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        bestKeys = self.getMaxKeys(livingGhostPositionDistributions)
        bestPos = None
        bestProb = 0
        for i in range(len(bestKeys)):
            if livingGhostPositionDistributions[i][bestKeys[i]] >= bestProb:
                bestPos = bestKeys[i]
                bestProb = livingGhostPositionDistributions[i][bestKeys[i]]

        bestAction = None
        minDist = float("inf")
        for a in legal:
            successor = Actions.getSuccessor(pacmanPosition, a)
            dist = self.distancer.getDistance(successor, bestPos)
            if dist < minDist:
                bestAction = a
                minDist = dist
        return bestAction
    ...
    ############################################################################
    # Goes through each util.Counter in d and finds the maximum prob for each
    # one and adds the key of that max(prob) to keys and returns keys
    ############################################################################
    def getMaxKeys(self, d):
        keys = []
        for item in d:
            minProb = 0
            bestKey = None
