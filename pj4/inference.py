    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            for pos in self.legalPositions:
                ...
                allPossible[pos] = emissionModel[dist] * distro[pos]
            for key in allPossible:
                if allPossible[key]:
                    nextParts = []
                    i = 0
                    while i < self.numParticles:
                        nextParts.append(util.sample(allPossible))
                        i += 1
                    self.particles = nextParts
                    return
            # If we get to this point, allPossible has all 0s and we should
